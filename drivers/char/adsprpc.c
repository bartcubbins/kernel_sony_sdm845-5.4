...
static int fastrpc_init_adsp_shared_heap(struct device *dev)
{
	int err = 0;
	struct fastrpc_apps *me = &gfa;
	struct smq_phy_page range;
...
static int fastrpc_probe(struct platform_device *pdev)
{
	...
	if (of_device_is_compatible(dev->of_node,
		...
		ret = fastrpc_init_adsp_shared_heap(dev);
		if (ret) {
			pr_warn("adsprpc: Error: %s: failed to assign ADSP heap to remote VMs with %d\n",
				__func__, ret);
			goto bail;
		}
