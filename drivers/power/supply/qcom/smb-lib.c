...
static const struct apsd_result smblib_apsd_results[] = {
	...
	[HVDCP2] = {
		...
		.val	= POWER_SUPPLY_TYPE_USB_HVDCP
	},
	[HVDCP3] = {
		.name	= "USB_HVDCP_3_CHARGER",
		.bit	= DCP_CHARGER_BIT | QC_3P0_BIT,
		.val	= POWER_SUPPLY_TYPE_USB_HVDCP_3,
	},
};
#else
static const struct apsd_result smblib_apsd_results[] = {
...
#if defined(CONFIG_SOMC_CHARGER_EXTENSION)
int smblib_somc_get_charger_type(struct smb_charger *chg)
{
	int charger_type;
	const struct apsd_result *apsd_result;

	if (chg->pd_active) {
		charger_type = POWER_SUPPLY_TYPE_USB_PD;
	} else {
		apsd_result = smblib_get_apsd_result(chg);
		charger_type = apsd_result->val;
	}
...
#if defined(CONFIG_SOMC_CHARGER_EXTENSION)
	[SMB2_FG_GEN3_SOMC_MONOTONIC_SOC] = "somc_monotonic_soc",
#endif
};
...
#if defined(CONFIG_SOMC_CHARGER_EXTENSION)
/* IDTP9221 channels */
static const char * const idtp9221_ext_iio_chan[] = {
	[IDTP9221_SOMC_AUTH] = "auth",
	[IDTP9221_SOMC_CHARGER_TYPE] = "charger_type",
	[IDTP9221_SOMC_USBIN_VALID] = "usbin_valid",
	[IDTP9221_SOMC_WIRELESS_STATUS] = "wireless_status",
	[IDTP9221_SOMC_WIRELESS_THERMAL_V_LIMIT] = "wireless_thermal_v_limit",
};
#endif
...
#if defined(CONFIG_SOMC_CHARGER_EXTENSION)
	apsd_result = smblib_get_apsd_result(chg);
	if (apsd_result->val == POWER_SUPPLY_TYPE_USB ||
				...
				int *val)
{
	...
	rc = smblib_get_prop_from_bms(chg, SMB2_FG_GEN3_TEMP, &pval.intval);
	if (rc < 0) {
	...
	if (chg->real_temp_use_aux) {
		rc = smblib_get_prop_skin_temp(chg, &pval.intval);
		if (rc < 0) {
...
static void smblib_somc_wlc_fake_charging_work(struct work_struct *work)
{
	...
	if (chg->wireless_psy) {
		if (IS_ERR_OR_NULL(chg->iio_chan_list_idtp9221))
			return;

		rc = iio_read_channel_processed(
				chg->iio_chan_list_idtp9221[IDTP9221_SOMC_WIRELESS_STATUS], &pval.intval);
		if (rc < 0) {
			smblib_dbg(chg, PR_SOMC,
				    ...
				    int val)
{
	...
	if (chg->wireless_psy) {
		...
		rc = iio_read_channel_processed(
				chg->iio_chan_list_idtp9221[IDTP9221_SOMC_AUTH], &pval.intval);
		if (rc < 0)
			smblib_dbg(chg, PR_SOMC,
				    ...
				    int *val)
{
	...
	rc = iio_read_channel_processed(chg->iio.skin_temp_chan, val);
	*val /= 100;
	return rc;
}
				    ...
				    int *val)
{
	...
	return iio_read_channel_processed(chg->iio.dcin_v_chan, val);
}
				    ...
				    int *val)
{
	...
	return iio_read_channel_processed(chg->iio.dcin_i_chan, val);
}
#endif
						...
						int *val)
{
	...
	*val = (reg & TYPEC_LEGACY_CABLE_STATUS_BIT) ? 1 : 0;
	return 0;
}
#endif
								...
								int max_voltage)
{
	...
	return iio_write_channel_raw(
			chg->iio_chan_list_idtp9221[IDTP9221_SOMC_WIRELESS_THERMAL_V_LIMIT],
			max_voltage);
}
#endif
...
static void smblib_legacy_detection_work(struct work_struct *work)
{
	...
	if (rc < 0)
		val.intval = 0;
	rp_default = chg->typec_mode == QTI_POWER_SUPPLY_TYPEC_SOURCE_DEFAULT;
	if (val.intval) {
...
static void smblib_somc_jeita_work(struct work_struct *work)
{
	...
	rc = smblib_get_prop_from_bms(chg, SMB2_FG_GEN3_VOLTAGE_NOW, &pval.intval);
	if (rc < 0) {
...
int smblib_init(struct smb_charger *chg)
{
	...
	chg->charger_removal_wakelock.lock =
				wakeup_source_register(chg->dev, "unplug_wakelock");
...
#if defined(CONFIG_SOMC_CHARGER_EXTENSION)
		iio_list = get_ext_channels(chg->dev, idtp9221_ext_iio_chan,
			ARRAY_SIZE(idtp9221_ext_iio_chan));
		if (!IS_ERR(iio_list))
			chg->iio_chan_list_idtp9221 = iio_list;
...
int smblib_deinit(struct smb_charger *chg)
{
	...
	if (chg->charger_removal_wakelock.enabled) {
		wakeup_source_unregister(chg->charger_removal_wakelock.lock);
...
void smblib_somc_ctrl_inhibit(struct smb_charger *chg, bool en)
{
	...
	rc = smblib_get_prop_from_bms(chg, SMB2_FG_GEN3_SOMC_MONOTONIC_SOC, &msoc);
	if (rc < 0) {
...
void smblib_somc_lrc_check(struct smb_charger *chg)
{
	...
	if (chg->iio_chan_list_fg_gen3) {
		rc = smblib_get_prop_from_bms(chg, SMB2_FG_GEN3_CAPACITY, &val.intval);
		if (rc < 0) {
			pr_err("Couldn't get capacity prop rc=%d\n", rc);
			goto exit;
		} else {
			soc = val.intval;
		}
...
void smblib_somc_handle_wireless_exclusion(struct smb_charger *chg)
{
	...
	if (!IS_ERR_OR_NULL(chg->iio_chan_list_idtp9221))
		iio_write_channel_raw(
			chg->iio_chan_list_idtp9221[IDTP9221_SOMC_USBIN_VALID],
			val.intval);
}
#endif
